...
const HistoryPage = () => {
  ...
  return (
    <DashboardLayout>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {formatDate(a.created_at)} • {items.length} aspectos analisados
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8"
                              onClick={(e) => { e.stopPropagation(); downloadAnalysisPDF(a); }}
                            >
      ...
      {/* Instagram Analysis Detail Dialog */}
      <Dialog open={!!selectedAnalysis} onOpenChange={(open) => !open && setSelectedAnalysis(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {selectedAnalysis && formatDate(selectedAnalysis.created_at)}
            </p>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            {selectedAnalysis &&
              (Array.isArray(selectedAnalysis.analysis) ? selectedAnalysis.analysis : []).map(
            ...
            <div className="flex justify-end">
              <Button
                ...
                size="sm"
                className="gap-2"
                onClick={() => selectedAnalysis && downloadAnalysisPDF(selectedAnalysis)}
              >
