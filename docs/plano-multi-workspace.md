...
**Status:** proposta para revisão — nenhuma migração deste plano foi aplicada.
**Data:** 2026-07-15
...
- Edge functions usam **service role** e validam o JWT manualmente; a autorização
  efetiva nelas é feita em código (filtros por `user_id`), não por RLS.
...
conversa), `content_generation_jobs`, `reference_documents`, `user_designs`,
...
Fases 2 e 3 podem inverter de ordem se o pessoal/institucional for mais urgente que o
multi-perfil — a fundação (Fase 1) serve às duas.
